...
class CartManager {
    ...
    async loadCart() {
        ...
        try {
            ...
            if (data.success) {
                ...
                this.showError('Không thể tải giỏ hàng: ' + (data.message || 'Unknown error'));
            }
        } catch (error) {
    ...
    renderCart() {
        ...
        // Update cart count
        document.getElementById('cart-count').textContent = this.cart.items.length;
        document.getElementById('select-all-count').textContent = this.cart.items.length;
        ...
        this.cart.items.forEach((item, index) => {
            ...
            checkbox.checked = this.selectedItems.has(item.item_id) || this.selectedItems.size === 0;
            ...
            const productUrl = item.product_slug 
                ? this.config.urls.productDetail.replace(':slug', item.product_slug)
                : '#';
            clone.querySelector('.cart-item-image a').href = productUrl;
            
            // Name
            const nameLink = clone.querySelector('.cart-item-name a');
            nameLink.textContent = item.product_name || `Sản phẩm #${item.product_id}`;
            ...
            qtyInput.addEventListener('change', (e) => {
                this.handleQuantityChange(item.item_id, item.product_id, parseInt(e.target.value));
            });
            ...
            clone.querySelector('.qty-minus').addEventListener('click', () => {
                ...
                if (newQty !== item.quantity) {
                    this.handleQuantityChange(item.item_id, item.product_id, newQty);
                }
            });
            ...
            itemTotal.textContent = this.formatCurrency(item.subtotal || item.price * item.quantity);
            ...
            console.log(`Item ${index} appended to list. Current items count:`, cartItemsList.children.length);
        });
    ...
    updateSummary() {
        const selectedItems = this.cart.items.filter(item => this.selectedItems.has(item.item_id));
        
        const subtotal = selectedItems.reduce((sum, item) => {
            return sum + (item.price * item.quantity);
        }, 0);

        document.getElementById('summary-items-count').textContent = selectedItems.length;
    ...
    updateSelectAllCheckbox() {
        ...
        const allSelected = this.cart.items.every(item => this.selectedItems.has(item.item_id));
        const someSelected = this.cart.items.some(item => this.selectedItems.has(item.item_id));
    ...
    async handleRemoveItem(itemId) {
        ...
        try {
            ...
            if (data.success) {
                ...
                this.showToast(data.message || 'Không thể xóa sản phẩm.', 'error');
            }
        } catch (error) {
    ...
    async handleDeleteSelected() {
        ...
        if (!confirm(`Bạn có chắc muốn xóa ${this.selectedItems.size} sản phẩm đã chọn?`)) return;
    ...
    async handleClearCart() {
        ...
        try {
            ...
            if (data.success) {
                ...
                this.showToast(data.message || 'Không thể xóa giỏ hàng.', 'error');
            }
        } catch (error) {
    ...
    handleCheckout() {
        ...
        sessionStorage.setItem('checkoutItems', JSON.stringify(Array.from(this.selectedItems)));
...
window.addToCart = async function(productId, quantity = 1) {
    try {
        const response = await fetch(window.cartConfig?.urls?.addItem || '/api/web/cart/items', {
            method: 'POST',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': window.cartConfig?.csrfToken || document.querySelector('meta[name="csrf-token"]')?.content,
            },
            body: JSON.stringify({
                product_id: productId,
                quantity: quantity,
...
async function updateHeaderCartCount() {
    try {
        const response = await fetch(window.cartConfig?.urls?.getCart || '/api/web/cart');
        const data = await response.json();
        ...
        if (data.success && data.data) {
            ...
            if (countEl) {
                ...
                countEl.style.display = data.data.total_quantity > 0 ? 'flex' : 'none';
            }
        }
    } catch (error) {
